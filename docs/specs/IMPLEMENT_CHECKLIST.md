...
- [ ] `POST /api/history/log` → CSV row append 확인 (payload 형식은 아래 참조)
- [ ] `GET /api/history` → 누적 CSV 전체 조회 → History 페이지 로드 시 자동 merge
- [ ] 네트워크 단절 후 복구 시 frontend pending queue가 순서대로 drain되는지 확인

`POST /api/history/log` payload는 CSV 저장을 위한 flat JSON 형식입니다:
...
- `snapshot_status`: `"captured"` 또는 `"unavailable"`
...
| History CSV append/fetch | ✅ 구현 완료 | 실환경 smoke test 필요 |
...
- [ ] `POST /api/history/log` CSV append
